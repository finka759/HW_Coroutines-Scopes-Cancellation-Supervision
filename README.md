...
Cтрока `throw Exception("something bad happened")` внутри первого `launch` не будет достигнута, потому что корутина будет отменена в момент `delay` из-за выброшенного исключения во втором `launch`.
    ...
    Ответ 2.5
Обе строки `throw Exception("something bad happened")` будут исполнены. Строка e.printStackTrace() внутри catch блока не будет выполнена, поскольку CoroutineScope(...).launch не позволяет внешнему try catch поймать исключение, а пробрасывает его в стандартный CoroutineExceptionHandler для JVM
...
Когда исключение происходит внутри корутины, запущенной с CoroutineScope(...).launch, это исключение не “перебрасывается” из функции launch так, чтобы его мог поймать внешний try-catch, но
вместо этого launch (которая является fire-and-forget операцией по умолчанию) отправляет эти исключения в Стандартный CoroutineExceptionHandler для JVM, который:

- логирует исключение в стандартный поток ошибок (stderr), что  и видно в консоли и выглядит как обычный Stack Trace;
- не прерывает работу всей программы, если это не фатальное исключение;
- не позволяет внешнему try-catch (который обернут вокруг CoroutineScope(...).launch { ... }) поймать это исключение.

  Вывод: `try-catch` вокруг launch с CoroutineScope(EmptyCoroutineContext) никогда не сработает для исключений, возникших внутри дочерних корутин, если они не были обработаны или выброшены наверх до того, как корутина отправит их в `CoroutineExceptionHandler`
